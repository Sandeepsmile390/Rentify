...
import { StyleSheet, Text, View, ScrollView, TextInput, TouchableOpacity, KeyboardAvoidingView, Platform, ActivityIndicator, Alert } from 'react-native';
import { authService, tenantService, chatService } from '../../services/api';
...
export default function TenantChatScreen() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (myProfile) {
        ...
        const myChat = chats.find(c => c.tenantId === myProfile.id);
        if (myChat) {
          setMessages(myChat.messages);
          await chatService.markSeen(myProfile.id, 'tenant');
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const chats = await chatService.getChats();
        const myChat = chats.find(c => c.tenantId === tenant.id);
        if (myChat) {
          setMessages(myChat.messages);
        }
      } catch (e) {}
  ...
  const handleSend = async () => {
    ...
    try {
      await chatService.sendMessage(tenant.id, 'tenant', msgText);
      // Refresh local list
      const chats = await chatService.getChats();
      const myChat = chats.find(c => c.tenantId === tenant.id);
      if (myChat) setMessages(myChat.messages);
    } catch (error) {
  ...
  const handleSendAttachment = async (type) => {
    ...
    try {
      ...
      await chatService.sendMessage(tenant.id, 'tenant', `Sent a ${type}`, name, type);
      // Refresh local list
      const chats = await chatService.getChats();
      const myChat = chats.find(c => c.tenantId === tenant.id);
      if (myChat) setMessages(myChat.messages);
